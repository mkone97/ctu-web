...
function Navbar() {
  ...
  return (
    <header
      className={` ${barlow.variable} relative z-30 flex justify-between items-center  md:px-0 md:pl-10 `}
    >
      <Image
        src={logo}
        alt="logo image"
      ...
      >
        {pages.map(({ title, href }, index) => (
          ...
          >
            <li
              className={`flex items-center mb-12 w-full transition border-r-2 border-transparent gap-x-3 [&:not(.active)]:hover:border-white/50 [&.active]:border
